...
class BinaryFilelistDataset(Dataset):
    ...
    def __init__(
        ...
        self.stride = getattr(self.config.task, 'stride', 1)
    ...
    def _load_metadata(self) -> None:
        ...
        # 确保一天的数据足够提取至少一个完整的序列
        if self.time_steps_per_day < self.total_seq_len:
            raise ValueError(
                f"time_steps_per_day ({self.time_steps_per_day}) 必须大于等于 "
    ...
    def _build_sequence_index(self) -> None:
        ...
        for file_idx, bin_file in enumerate(self.valid_files):
            ...
            # 计算当前文件（一天）内可以提取的最后一个有效起始索引
            max_start_idx = self.time_steps_per_day - self.total_seq_len
            
            # 步长设为 stride，实现滑动窗口重叠切分
            for seq_idx in range(0, max_start_idx + 1, self.stride):
